...
import uuid
import os
from main import Workflow, TestCase, GelState
...
def list_workflows():
    ...
    for i, workflow in enumerate(workflows):
        with st.expander(f"{workflow.workflow_name or 'Untitled'} ({workflow.workflow_id})"):
...
def edit_workflow():
    ...
    workflow_description = st.text_area("Workflow Description", value=workflow.description or "")
        ...
        for i, test_case in enumerate(workflow.test_cases):
            ...
            with col2:
                if st.button("↑", key=f"test_up_{i}") and i > 0:
                    ...
                    workflow.test_cases[i], workflow.test_cases[i-1] = workflow.test_cases[i-1], workflow.test_cases[i]
            ...
            with col3:
                if st.button("↓", key=f"test_down_{i}") and i < len(workflow.test_cases) - 1:
            ...
            with st.expander(f"Edit Test Case {i+1}"):
                ...
                # Display prompt
                prompt = st.text_area(f"Prompt", value=test_case.prompt or "", key=f"prompt_{i}")
                
                # Initial State
                st.subheader("Initial State")
                initial_description = st.text_input("Description", 
                                                 value=test_case.initial_state.description if test_case.initial_state else "", 
                                                 key=f"initial_description_{i}")
                
                # Use monospace for schema
                st.write("Schema (code):")
                initial_schema = st.text_area("", 
                                           value=test_case.initial_state.schema if test_case.initial_state else "", 
                ...
                initial_data = st.text_area("", 
                                         value=test_case.initial_state.data if test_case.initial_state else "", 
                ...
                if steps_key not in st.session_state:
                    st.session_state[steps_key] = test_case.expected_steps.copy() if test_case.expected_steps else []
                ...
                for step_idx, step in enumerate(st.session_state[steps_key]):
                    ...
                    with col1:
                        new_step = st.text_input(f"Step {step_idx+1}", value=step, key=f"step_{i}_{step_idx}")
                        if new_step != step:
                            st.session_state[steps_key][step_idx] = new_step
                    
                    with col2:
                        if st.button("↑", key=f"up_{i}_{step_idx}") and step_idx > 0:
                            st.session_state[steps_key][step_idx], st.session_state[steps_key][step_idx-1] = st.session_state[steps_key][step_idx-1], st.session_state[steps_key][step_idx]
                            st.rerun()
                    
                    with col3:
                        if st.button("↓", key=f"down_{i}_{step_idx}") and step_idx < len(st.session_state[steps_key]) - 1:
                            st.session_state[steps_key][step_idx], st.session_state[steps_key][step_idx+1] = st.session_state[steps_key][step_idx+1], st.session_state[steps_key][step_idx]
                            st.rerun()
                ...
                outcome_description = st.text_input("Description", 
                                                 value=test_case.expected_outcome.description if test_case.expected_outcome else "", 
                                                 key=f"outcome_description_{i}")
                
                # Use monospace for schema
                st.write("Schema (code):")
                outcome_schema = st.text_area("", 
                                           value=test_case.expected_outcome.schema if test_case.expected_outcome else "", 
                ...
                outcome_data = st.text_area("", 
                                         value=test_case.expected_outcome.data if test_case.expected_outcome else "", 
                ...
                if st.button("Save Test Case Changes", key=f"save_test_case_{i}"):
                    ...
                    workflow.test_cases[i].expected_steps = [step for step in st.session_state[steps_key] if step]
...
def add_test_case():
    ...
    for step_idx, step in enumerate(st.session_state.new_test_case_steps):
        ...
        with col1:
            new_step = st.text_input(f"Step {step_idx+1}", value=step, key=f"new_step_{step_idx}")
            if new_step != step:
                st.session_state.new_test_case_steps[step_idx] = new_step
        
        with col2:
            if st.button("↑", key=f"new_up_{step_idx}") and step_idx > 0:
                st.session_state.new_test_case_steps[step_idx], st.session_state.new_test_case_steps[step_idx-1] = st.session_state.new_test_case_steps[step_idx-1], st.session_state.new_test_case_steps[step_idx]
                st.rerun()
        
        with col3:
            if st.button("↓", key=f"new_down_{step_idx}") and step_idx < len(st.session_state.new_test_case_steps) - 1:
                st.session_state.new_test_case_steps[step_idx], st.session_state.new_test_case_steps[step_idx+1] = st.session_state.new_test_case_steps[step_idx+1], st.session_state.new_test_case_steps[step_idx]
                st.rerun()
    ...
    if st.button("Save Test Case"):
        test_case = TestCase(
            ...
            initial_state=GelState(
                ...
                schema=initial_schema,
                data=initial_data
            ) if any([initial_description, initial_schema, initial_data]) else None,
            expected_steps=[step for step in st.session_state.new_test_case_steps if step],
            expected_outcome=GelState(
                description=outcome_description,
                schema=outcome_schema,
                data=outcome_data
            ) if any([outcome_description, outcome_schema, outcome_data]) else None
        )
